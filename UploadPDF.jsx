...
function parsePDFText(text) {
  console.log("🔍 [DEBUG] parsePDFText() appelée")
  console.log("🔍 [DEBUG] Longueur du texte:", text.length)
  
  const lines = text.split('\n').map(l => l.trim()).filter(Boolean)
  console.log("🔍 [DEBUG] Nombre de lignes trouvées:", lines.length)
  console.log("🔍 [DEBUG] Premières 10 lignes:", lines.slice(0, 10))
  ...
  console.log("🔍 [DEBUG] Months normalized:", monthsNormalized)
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (dateMatch) {
      console.log("✅ [DEBUG] DATE TROUVÉE à la ligne", i, ":", line)
      console.log("✅ [DEBUG] Capture complète:", dateMatch)
      const day = parseInt(dateMatch[1])
      const rawMonth = dateMatch[2]
      const monthStr = removeAccents(rawMonth.toLowerCase())
      console.log("✅ [DEBUG] Day:", day, "| Raw month:", rawMonth, "| Normalized:", monthStr)
      const month = monthsNormalized[monthStr]
      console.log("✅ [DEBUG] Month number:", month)
      if (month) {
        const year = new Date().getFullYear()
        currentDate = `${year}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`
        console.log("✅ [DEBUG] DATE FINALE:", currentDate)
        if (!pdfDateDetected) pdfDateDetected = currentDate
      } else {
        console.error("❌ [DEBUG] ERREUR: Mois non trouvé dans monthsNormalized!", monthStr)
      }
    }
  ...
  console.log("🔍 [DEBUG] pdfDateDetected FINAL:", pdfDateDetected)
  console.log("🔍 [DEBUG] Tours trouvées:", Object.keys(tours))
...
export default function UploadPDFPage() {
  ...
  const handleFileAnalyze = async file => {
    try {
      console.log("🚀 [DEBUG] handleFileAnalyze() appelée avec fichier:", file.name)
      setProgress('Extraction du PDF...')
      const text = await extractTextFromPDF(file)
      console.log("🚀 [DEBUG] Texte extrait, longueur:", text.length)
      const { tours, detectedDate } = parsePDFText(text)
      console.log("🚀 [DEBUG] Parsing terminé")
      console.log("🚀 [DEBUG] detectedDate retourné:", detectedDate)
      console.log("🚀 [DEBUG] tours trouvées:", tours.length)
      
      if (!detectedDate) {
        console.error("❌ [DEBUG] ATTENTION: detectedDate est null/undefined!")
      } else {
        console.log("✅ [DEBUG] Date détectée avec succès:", detectedDate)
      }
      ...
      console.log("🚀 [DEBUG] Tournées non reconnues:", unmatched.length)
      setUnmatchedTours(unmatched.map(t => ({ ...t, manualName: '' })))
    } catch (error) {
      console.error("❌ [DEBUG] ERREUR dans handleFileAnalyze:", error)
  ...
  const handleUpload = async () => {
    console.log("📤 [DEBUG] handleUpload() appelée")
    console.log("📤 [DEBUG] file:", file?.name)
    console.log("📤 [DEBUG] detectedDate:", detectedDate)
    
    if (!file || !detectedDate) {
      console.error("❌ [DEBUG] ERREUR: file ou detectedDate est vide!", { file: file?.name, detectedDate })
    ...
    try {
      ...
      if (!parsedDate) {
        console.error("❌ [DEBUG] ERREUR: parsedDate retournée est null!")
